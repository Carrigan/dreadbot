...
const BASIC_LAND_NAMES: &'static [&'static str] =
    &["Swamp", "Island", "Forest", "Mountain", "Plains"];
...
pub fn request_pricing(deck: &Deck) -> Result<Vec<PricingSource>, Box<dyn std::error::Error>> {
    ...
    // Send it and merge
    let mut response: ScryfallResponse = reqwest::Client::new().get(&query).send()?.json()?;

    data.append(&mut response.data);

    // Consume until there is no more
    while let Some(next_url) = response.next_page {
        response = reqwest::Client::new().get(&next_url).send()?.json()?;
...
fn test_double_sided_card_requests() {
    ...
    assert_eq!(
        scryfall_resp.get(0).unwrap().name,
        "Delver of Secrets // Insectile Aberration"
    );
    assert_eq!(
        scryfall_resp.get(0).unwrap().front_name,
        "Delver of Secrets"
    );
}
